/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.phucdk.emailsender.object;

/**
 *
 * @author Administrator
 */
public class Canho {

    private long STT;
    private String soCanho;
    private String tang;
    private String dientichSudung;
    private String dientichXaydung;
    private String heso;
    private String giatriHeso;
    private String dongia;
    private String giabanThongthuy;
    private String giatriCanhoHoanthien;
    private String tongGiatriHopdong;
    private String tongGiatriPhainopTheotiendo;
    private String soDanop;
    private String soConphainopDotnay;
    private String dot1;
    private String hovatenKhachang;
    private String soChungminh;
    private String ngaycap;
    private String noicap;
    private String hokhauThuongtru;
    private String diachiLienhe;
    private String dienthoai;
    private String email;
    private String sotienBangchu;

    public long getSTT() {
        return STT;
    }

    public void setSTT(long STT) {
        this.STT = STT;
    }

    public String getSoCanho() {
        return soCanho;
    }

    public void setSoCanho(String soCanho) {
        this.soCanho = soCanho;
    }

    public String getTang() {
        return tang;
    }

    public void setTang(String tang) {
        this.tang = tang;
    }

    public String getDientichSudung() {
        return dientichSudung;
    }

    public void setDientichSudung(String dientichSudung) {
        this.dientichSudung = dientichSudung;
    }

    public String getDientichXaydung() {
        return dientichXaydung;
    }

    public void setDientichXaydung(String dientichXaydung) {
        this.dientichXaydung = dientichXaydung;
    }

    public String getHeso() {
        return heso;
    }

    public void setHeso(String heso) {
        this.heso = heso;
    }

    public String getGiatriHeso() {
        return giatriHeso;
    }

    public void setGiatriHeso(String giatriHeso) {
        this.giatriHeso = giatriHeso;
    }

    public String getDongia() {
        return dongia;
    }

    public void setDongia(String dongia) {
        this.dongia = dongia;
    }

    public String getGiabanThongthuy() {
        return giabanThongthuy;
    }

    public void setGiabanThongthuy(String giabanThongthuy) {
        this.giabanThongthuy = giabanThongthuy;
    }

    public String getGiatriCanhoHoanthien() {
        return giatriCanhoHoanthien;
    }

    public void setGiatriCanhoHoanthien(String giatriCanhoHoanthien) {
        this.giatriCanhoHoanthien = giatriCanhoHoanthien;
    }

    public String getTongGiatriHopdong() {
        return tongGiatriHopdong;
    }

    public void setTongGiatriHopdong(String tongGiatriHopdong) {
        this.tongGiatriHopdong = tongGiatriHopdong;
    }

    public String getTongGiatriPhainopTheotiendo() {
        return tongGiatriPhainopTheotiendo;
    }

    public void setTongGiatriPhainopTheotiendo(String tongGiatriPhainopTheotiendo) {
        this.tongGiatriPhainopTheotiendo = tongGiatriPhainopTheotiendo;
    }

    public String getSoDanop() {
        return soDanop;
    }

    public void setSoDanop(String soDanop) {
        this.soDanop = soDanop;
    }

    public String getSoConphainopDotnay() {
        return soConphainopDotnay;
    }

    public void setSoConphainopDotnay(String soConphainopDotnay) {
        this.soConphainopDotnay = soConphainopDotnay;
    }

    public String getDot1() {
        return dot1;
    }

    public void setDot1(String dot1) {
        this.dot1 = dot1;
    }

    public String getHovatenKhachang() {
        return hovatenKhachang;
    }

    public void setHovatenKhachang(String hovatenKhachang) {
        this.hovatenKhachang = hovatenKhachang;
    }

    public String getSoChungminh() {
        return soChungminh;
    }

    public void setSoChungminh(String soChungminh) {
        this.soChungminh = soChungminh;
    }

    public String getNgaycap() {
        return ngaycap;
    }

    public void setNgaycap(String ngaycap) {
        this.ngaycap = ngaycap;
    }

    public String getNoicap() {
        return noicap;
    }

    public void setNoicap(String noicap) {
        this.noicap = noicap;
    }

    public String getHokhauThuongtru() {
        return hokhauThuongtru;
    }

    public void setHokhauThuongtru(String hokhauThuongtru) {
        this.hokhauThuongtru = hokhauThuongtru;
    }

    public String getDiachiLienhe() {
        return diachiLienhe;
    }

    public void setDiachiLienhe(String diachiLienhe) {
        this.diachiLienhe = diachiLienhe;
    }

    public String getDienthoai() {
        return dienthoai;
    }

    public void setDienthoai(String dienthoai) {
        this.dienthoai = dienthoai;
    }

    public String getEmail() {
        return email;
    }

    public void setEmail(String email) {
        this.email = email;
    }

    public String getSotienBangchu() {
        return sotienBangchu;
    }

    public void setSotienBangchu(String sotienBangchu) {
        this.sotienBangchu = sotienBangchu;
    }
    
    
    
}
